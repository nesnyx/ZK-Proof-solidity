...
void IsOver18_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void IsOver18_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 19;}
...
ctx->componentMemory[coffset].templateName = "IsOver18";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
GreaterThan_2_create(mySignalStart+2,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
uint cmp_index_ref = 0;
...
IsOver18_3_create(1,0,ctx,"main",0);
IsOver18_3_run(0,ctx);
}
